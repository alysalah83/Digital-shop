...
import { ProductWithReviews } from "../types/product.type";
...
function ProductWithDetails({ product }: { product: ProductWithReviews }) {
  const { id, image, name, discountPercentage, reviews, stock, rating, price } =
    product;
  ...
  return (
    <div className="gap mt-6 flex flex-col gap-10 lg:flex-row">
      ...
      <div className="sm:p-4 lg:min-w-1/2 lg:py-8">
        ...
        <div>
          <ProductOptions render={<ProductStockCount stock={stock} />} />
        </div>
      </div>
    </div>
  );
